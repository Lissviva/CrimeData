...
with col7:
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=dict(text='Total Crimes', font=dict(color='red')),
            tickfont=dict(color='red')
        ),
        yaxis2=dict(
            title=dict(text='Economic Inactivity Rate (%)', font=dict(color='blue')),
...
with tab5:
    ...
    with col9:
        ...
        def load_prediction_probabilities():
            df = pd.read_csv('crime_prediction_with_probabilities.csv')
            df['District'] = df['District'].str.strip()
            return df

        proba_df = load_prediction_probabilities()
